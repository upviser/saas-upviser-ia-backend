    ...
    try {
        ...
        if (!req.body.agent || shopLogin.conversationsAI < 1) {
            ...
            if (JSON.stringify(type.output_parsed).toLowerCase().includes('agendamientos')) {
                ...
                information = `${information}. ${JSON.stringify(simplifiedCalls).replaceAll('"', '')}. Si el usuario quiere agendar una llamada identifica la llamada más adecuada y pon su enlace de esta forma: <a href="/llamadas/Nombre%20de%20la%20llamada">Nombre de la llamada</a> utilizando el call.nameMeeting`
            }
