...
export const createWebhook = async (req, res) => {
    ...
    const storeData = await StoreData.findById('6829762596c6028092e4aa35').lean()
...
export const getMessage = async (req, res) => {
    try {
        ...
        if (req.body?.entry && req.body.entry[0]?.changes && req.body.entry[0].changes[0]?.value?.messages && 
            ...
            const tenant = await Tenant.findOne({ instagramId: req.body.entry[0].id }).lean()
            if (tenant) {
                const tenantId = tenant.id
                const integration = await Integration.find().lean()
    ...
    - Responde de manera breve y clara`},
            ...
            const tenant = await Tenant.findOne({ messengerId: req.body.entry[0].id }).lean()
            if (tenant) {
                const tenantId = tenant.id
                const integration = await Integration.find().lean()
                const domain = await Domain.findOne({ tenantId }).lean()
                ...
                if (integration.messengerToken) {
                    ...
                    if ((messages && messages.length && messages[0].agent) || shopLogin.conversationsAI < 1) {
                        ...
                        if (JSON.stringify(type.output_parsed).toLowerCase().includes('agendamientos')) {
                            ...
                            information = `${information}. ${JSON.stringify(simplifiedCalls).replaceAll('"', '')}. Si el usuario quiere agendar una llamada identifica la llamada más adecuada y pon su link de esta forma: ${domain.domain === 'upviser.cl' ? process.env.WEB_URL : `https://${domain.domain}`}/llamadas/Nombre%20de%20la%20llamada utilizando el call.nameMeeting`
                        }
                        if (JSON.stringify(type.output_parsed).toLowerCase().includes('intención de compra de productos')) {
                            let cart
    ...
    2. "message": un texto natural para enviar al usuario.  
    - Sigue preguntando qué más desea hasta que diga todo lo que quiere comprar.  
    - Si ya está listo para pagar, comparte este enlace: ${domain?.domain === 'upviser.cl' ? process.env.WEB_URL : `https://${domain.domain}`}/finalizar-compra?messengerId=${sender}
    - Responde de manera breve y clara`},
                                    ...conversation,
                                    {"role": "user", "content": message}
                                ],
                ...
                const tenant = await Tenant.findOne({ instagramId: req.body.entry[0].id })
                if (tenant) {
                    const tenantId = tenant.id
                    const integration = await Integration.find().lean()
                    const domain = await Domain.findOne({ tenantId }).lean()
        ...
        2. "message": un texto natural para enviar al usuario.  
        - Sigue preguntando qué más desea hasta que diga todo lo que quiere comprar.  
        - Si ya está listo para pagar, comparte este enlace: ${domain?.domain === 'upviser.cl' ? process.env.WEB_URL : `https://${domain.domain}`}/finalizar-compra?instagramId=${sender}
        - Responde de manera breve y clara`},
                                        ...conversation,
                                        {"role": "user", "content": message}
                                    ],
...
export const deleteData = async (req, res) => {
    try {
        ...
        if (integrations) {
            ...
            if (tenant) {
                if (tenant.idInstagram === userId) {
                    await Tenant.findByIdAndUpdate(integrations._id, { idInstagram: '' })
                } else if (tenant.idPage === userId) {
                    await Tenant.findByIdAndUpdate(integrations._id, { idPage: '', })
                } else if (tenant.idPhone === userId) {
                    await Tenant.findByIdAndUpdate(integrations._id, { idPhone: '' })
                }
            }
        }
        const confirmationCode = `del_${userId}_${Date.now()}`;
        const statusUrl = `${process.env.API_URL}/delete-status?code=${confirmationCode}`;
        res.status(200).json({
            url: statusUrl,
